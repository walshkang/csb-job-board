...
const COMPANY_CANONICAL_ALIASES = {
  ...
  'Last Financing Date': ['Last Financing Date', 'Last Financing', 'Financing Date'],
...
async function main() {
  ...
  function companyToProps(c, resolvedNameMap, databaseId) {
    const canonicalProps = {
      ...
      'Last Financing Date': (() => { const f = Array.isArray(c.funding_signals) && c.funding_signals[0]; return f && f.date ? { rich_text: [{ text: { content: String(f.date) } }] } : undefined; })(),
